...
% result_map
params_group = values(result_map, {'mt_cons', 'mt_incm', 'mt_val', 'mt_pol_a'});
[mt_cons, mt_incm, mt_val, mt_pol_a] = params_group{:};
...
if (bl_graph_pol_pct)
    ...
    for sub_j=1:1:2
        ...
        if(sub_j==1)
            ...
            mt_outcome(~mt_it_borr_idx) = mt_pol_a(~mt_it_borr_idx)./mt_a(~mt_it_borr_idx);
            st_y_label = 'br/bar(B) if br; sv/A if sv';
            st_legend_loc = 'northwest';
        end
        if(sub_j==2)
            mt_outcome(mt_it_borr_idx) = mt_cons(mt_it_borr_idx)./(mt_a(mt_it_borr_idx) + mt_pol_a(mt_it_borr_idx));
            mt_outcome(~mt_it_borr_idx) = mt_cons(~mt_it_borr_idx)./mt_a(~mt_it_borr_idx);
            st_y_label = 'c/(a+b) if br; c/a if sv';
            st_legend_loc = 'northwest';
        end
        ...
        grid on;
        legendCell = cellstr(num2str(ar_z', 'shock=%3.2f'));
        title([st_title_prefix 'Asset, Shocks and Choices (Perc)'])
        ylabel(st_y_label)
        xlabel({'Asset'})
