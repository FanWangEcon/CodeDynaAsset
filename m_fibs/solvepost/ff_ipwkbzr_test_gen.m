...
%%
function result_map = ff_ipwkbzr_test_gen(varargin)
...
for it_param_test_ctr = 1:length(ar_it_test_grp)
    
    it_test_grp = ar_it_test_grp(it_param_test_ctr);
    ...
    for it_cur_param = 1:1:length(ar_param_values)
        ...
        tb_outcomes_meansdperc_wthinfo.Properties.RowNames = ...
            strcat(variablenames, '_p', num2str(it_param_test_ctr), 'v', num2str(it_cur_param));
        ...
        if (it_param_test_ctr == 1 && it_cur_param == 1)
...
%     cl_tb_outcomes_meansdperc_wthinfo{it_param_test_ctr} = tb_outcomes_meansdperc_wthinfo_stack;

    %% Reset Base Parameters, parameters already grabbed out, updating
    % Needed if only changing one parameter at a time, holding others at
    % initial values. 
