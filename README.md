...
For an introduction to two period savings and borrowing problem and the demand and supply for credit, see [Fan](https://fanwangecon.github.io)'s [Intro Mathematics for Economists](https://fanwangecon.github.io/Math4Econ/).
...
This section solves the standard household-side problem of the [Bewley](https://en.wikipedia.org/wiki/Truman_Bewley)/[Aiyagari](https://en.wikipedia.org/wiki/S._Rao_Aiyagari) type [heterogeneous agent](https://en.wikipedia.org/wiki/Heterogeneity_in_economics) model. The supply curve for credit in the [Aiygari (1994)](https://www.jstor.org/stable/2118417#metadata_info_tab_contents) can be derived by invoking the programs for *1.1* and *1.2* with different interest rates.

Specifically, *1.1* derives the optimal conditional savings choices given interest rate, and *1.2* computes the stationary distributions over asset induced by the model parameters. Statistically, given discretization, the programs here provide the data-generating process for a joint discrete probability mass function over asset and shocks induced by price, preference, shock and discretization [parameters](https://fanwangecon.github.io/CodeDynaAsset/m_az/paramfunc/html/ffs_az_set_default_param.html).
...
Solve for the stationary probability mass function over states using *non-simulation* methods. The first two methods below involve *iteration*, the third is *semi-analytical and iteration-free* (see appendix in [Wang 2019](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3316939) for details). The semi-analytical approach has three implementations, **eigenvector** or **projection** or **nth-power**, and provides the exact stationary distribution. The **eigenvector** approach, when implemented with sparse matrix, is very fast. The distributional codes compute, with [benchmark parameters](https://fanwangecon.github.io/CodeDynaAsset/m_az/paramfunc/html/ffs_az_set_default_param.html) and after invoking the *optimized-vectorized* code from above, for each outcome *y*:
...
Algorithms 1 and 2 below provide identical solutions, 3 is the exact pmf of the dgp (speed below does not include speed from *1.1*):
